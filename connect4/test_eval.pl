...
testv1(X,Y,Color,CouleurVictoire):- (win(X, Y, rouge),pawn(X, Y, rouge),display,clear,CouleurVictoire = rouge); (win(X, Y, jaune),pawn(X, Y, jaune),display,clear,CouleurVictoire = jaune);(matchnull,display,CouleurVictoire = none,write('null'),clear).
testv1(X,Y,Color,CouleurVictoire):- Color == rouge,iarandom(X1,Y1),display,testv1(X1,Y1,jaune,CouleurVictoire).
testv1(X,Y,Color,CouleurVictoire):- Color == jaune,play2(X1,Y1),display,testv1(X1,Y1,rouge,CouleurVictoire).
